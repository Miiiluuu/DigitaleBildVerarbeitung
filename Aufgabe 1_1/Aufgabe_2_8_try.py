...
def drehung(image, grad, pixel_mitte):
    ...
    for x in range(-pixel_mitte, pixel_mitte):
        for y in range(-pixel_mitte, pixel_mitte):
            ...
            x_rotate = np.int(round((x) * np.cos(grad_rad) +
                                    (y) * -np.sin(grad_rad)))
            # Rotationsmatrix auf alle y-Werte anwenden
            y_rotate = np.int(round((x) * np.sin(grad_rad) +
                                    (y) * np.cos(grad_rad)))
...
def main():
    ...
    ax1, ax2 = plot_vorbereitung('Frequenzraum - Leistungsspektrum',
    ...
    ax2.imshow(power_rotate, cmap='gray', norm=LogNorm(),
               extent=[-0.5, 0.5, -0.5, 0.5])
