...
def erstelle_grauwerthist(werte, herkunft):
    ...
    fig = plt.figure(figsize=(10, 5))
    # Hinzufuegen der Ueberschrift zum Plot
    fig.suptitle(f'''Grauwerthistogramm fuer {herkunft}''', fontsize=16)
    # Hinzufuegen einer Unterueberschrift
    plt.title('mit gekuerzter Ordinatenachse')
    # Achsenbeschriftungen
    plt.xlabel(r'$f$')
    plt.ylabel('Häufigkeitsverteilung $h(f)$')
...
def mittelwert_grauwerthist(ordinate, abszisse):
    ...
    print(f'''Bei dem Mittelwert des Grauwert-Histogramms vom ''' +
          f'''Szintigramm handelt es sich um den Grauwert ''' +
          f'''{np.round(avg_hist, 3)}.''')
    return avg_hist
...
def calc_schwerpkt(image, calc_geometric=False):
    ...
    # Umrechnung in globales Koordinatensystem
    schwerpkt_x -= 128
    # Runden
    schwerpkt_x = np.round(schwerpkt_x, 3)
    # Berechnung vom Schwerpunkt in y-Richtung
    image_t = np.transpose(image)
    schwerpkt_y = calc_schwerpkt_1d(image_t)
    # Umrechnung in globales Koordinatensystem
    schwerpkt_y = ((schwerpkt_y - 128) * -1)
    # Runden
    schwerpkt_y = np.round(schwerpkt_y, 3)
...
def plot_schnittpkt(ueberschrift, abszisse, ordinate, image, schnittpkt):
    ...
    # Hinzufuegen der Ueberschrift zum Plot
    plt.suptitle(ueberschrift, fontsize=16)
    plt.imshow(image, cmap='gray', extent=[-128, 0, -128, 128])
    # Achsenbeschriftungen
    plt.xlabel(abszisse)
    plt.ylabel(ordinate)
...
def aufgabe_1_1(szinti):
    ...
    plot("Szintigramm aus Aufgabe 1.1", r'$x/mm$', r'$y/mm$', szinti)
...
def aufgabe_2_1(szinti, pixel_quadrant):
    ...
    # laengs y = 60, mit Umrechnung globales (entsprechend Vorlesungsskript)/
    # lokales (entsprechend numpy-Notation) Koordinatensystem
...
def aufgabe_2_8(szinti, pixel_quadrant):
    ...
    ax3.imshow(power, cmap='gray', norm=LogNorm(),
               extent=[-0.5, 0.5, -0.5, 0.5])
    # Plot gedrehtes Leistungsspektrum 
...
def aufgabe_2_11(szinti, pixel_quadrant):
    ...
    print("")
    # Erzeugung Bandpassfilter (in Groeße des Originalbildes)
    filter_bp = bandpassfilter(szinti, (3 / 4), 1, pixel_quadrant)
    # Anwendung des Bandpassfilters auf Originalbild
    szinti_gefiltert = use_filter(szinti, filter_bp)
    # Skalieren der Zahlenwerte, sodass Grauwerte von 0...255 umfasst werden
    szinti_gefiltert = make_scale(szinti_gefiltert)
    # Erstellung Plots fuer graphische Darstellung Originalbild und
    # gefiltertes Bild
...
def aufgabe_3_3(szinti, pixel_quadrant):
    ...
    # Erstellung Grauwertprofile entlang y-Linie = - 60 mm:
    ...
    bilder = [szinti, szinti_filter_avg, szinti_filter_med,
              szinti_filter_bin, grauprofil_szinti, grauprofil_avg,
    ...
    # Grauwertprofile der einzelnen Filter zum Vergleich in einer Figure
    # dargestellt
    fig = plt.figure(figsize=(6, 7))
    # Hinzufuegen der Ueberschrift zum Plot
    fig.suptitle("Vergleich Grauwertprofile der einzelnen Filter", fontsize=16)
    # Hinzufuegen einer Unterueberschrift
    plt.title('- laengs y = - 60 mm -')
    # Achsenbeschriftungen
    plt.xlabel(r'$x/mm$')
    plt.ylabel('Grauwert')
